...
首页以原有二次元插画为主体，使用奶油粉背景、柔和的玫瑰色与圆润卡片。桌面端将人物与文字分开排布，手机端使用上图下文。保留三张入口插画和暗号校验，不在公开首页展示文章信息。
...
默认显示浅色，仍保留主题原生深浅色切换和已有用户偏好。阅读页使用稳定的正文底色，没有新增第三方运行依赖。

暗色阅读采用中性炭灰底、柔和灰白正文和低饱和语法高亮；链接、行内代码与代码注释保持清晰，表格边线和配图亮度适度降低。目录、娜娜莉和音乐面板沿用同一套暗色色板，原生阅读模式也使用统一的底色与正文颜色。浅色主题保持奶油粉风格。
...
两项样式分别放在 `source/css/nanaly-delight.css`、`source/css/music-stage.css`，与粉色基础主题一起加载。
