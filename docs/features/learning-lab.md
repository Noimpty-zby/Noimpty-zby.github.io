...
入口 `/learn/`；文章页的「边学边练」将原页面左右分屏，可拖动分隔条调整宽度，窄屏用「阅读文章 / 编写代码」切换。分屏打开时按文章选语言：先看分类（Git、Linux 入门），再看正文里最多的代码块语言；随笔等没有代码的文章保持上次的语言。关闭或 PJAX 跳转时恢复原页面。输入与输出按文本渲染，不解释后端输出中的 HTML；终端输出交给 xterm.js 按终端字符处理。

## 使用

1. 解锁页面，点击顶部「连接后端」填写个人后端地址与令牌。状态点显示未连接、连接中、已连接或连接失败，验证成功后才变绿。
2. 选择 C、C++、Go、Python、Git、Linux 或 MySQL 后直接写代码。Git 和 Linux 默认打开**终端**（见下一节），工具栏的「终端 / 脚本」可以切回编辑器写整段脚本，选择按语言记在本浏览器。编辑器提供语法高亮与补全；不需要选择题目或载入实例。Python 环境已装好 NumPy 与 CPU 版 PyTorch，运行环境不联网、不能 pip install，第一次 `import torch` 需要几秒。
3. 在「输入」填写标准输入，点击「运行」或按 `Ctrl / ⌘ + Enter`；Git/Linux 按钮为「执行脚本」，每次执行整个编辑器内容。真实输出、错误、当前目录及退出码显示在「输出」，历史在「记录」中恢复。MySQL 直接执行编辑器里的 SQL。
...
5. 编辑器和输出区之间的横条可以拖动（或聚焦后按上下方向键）调整输出区高度，本浏览器记住高度，双击恢复默认。

## 终端（Git / Linux）

真实的交互式 Bash：逐条输入命令、回车就执行，提示符是 `learner@nanaly:~$`。可以用 `nano`、`vim` 编辑文件，`↑ ↓` 翻命令历史（`history`、`!!`、`Ctrl+R` 都能用），`Tab` 补全文件名和 Git 子命令，`Ctrl+C` 中断，`Ctrl+L` 清屏。选中文字后 `Ctrl+C` 是复制，`Ctrl+V` 粘贴。手机上终端下方有一排 Tab、Esc、Ctrl+C、Ctrl+D 和方向键。

- 文件、当前目录、导出的变量、别名、函数和命令历史都会保存：输入 `exit`、关闭页面、切走语言或切到「脚本」都会先保存再关闭。下次打开从上次停下的地方继续，脚本模式也接着同一个工作区。
- 连接断开或会话结束后，终端里会说明原因，按任意键重新打开；工具栏上的圆点绿色表示已连接。
- 闲置 30 分钟或连续使用 3 小时会自动保存并关闭；后端更新时同样先保存。
- 同一个工作区同时只能开一个终端：在另一个窗口打开时，旧窗口的会话先保存再交出。终端开着时，同一工作区的脚本要等终端关闭（页面里点「执行脚本」会自动先保存并关闭终端）。
- 右侧只有终端时，「请教娜娜莉」能看到终端最后约 60 行。
...
Linux 已提供 `ncal` / `cal`、常用文本工具、压缩解压、`jq`、`bc` 和 `man`；Linux 与 Git 中新建仓库都能使用默认身份提交。脚本模式一次跑完整段命令，没有键盘输入，`nano`、`vim`、`less` 这类交互程序请在终端里用。两种模式都无网络、非 root，`curl`、`wget`、`ssh` 虽已安装也不能连接外网；密码提示和长期后台服务不在支持范围内。
...
- `POST /api/terminal/ticket` 取一次性票据，再连 `GET /api/terminal?ticket=…` 的 WebSocket；消息格式见 `learning-backend.md`「交互式终端」。
