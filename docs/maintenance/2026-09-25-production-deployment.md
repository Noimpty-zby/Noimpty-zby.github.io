...
私有后端 `https://api.noimpty-zby.cn` 已部署并通过公网 HTTPS 与真实代码执行验收。前端提交 `8232297` 已在旧 GitHub Pages 网址成功发布，公开备份页及学习台可访问，线上脚本与发布版本一致。用户随后确认没有需要迁移的本机数据，明确跳过备份；当前准备将博客正式域名切换为 `https://noimpty-zby.cn`，需完成 Pages 域名绑定、DNS 与证书验收。
...
- 博客 URL、后台任务/日报地址、文章检查目标与链接检查来源统一准备为 `https://noimpty-zby.cn`，保留旧 Pages 历史结果的严格来源白名单。
- 15 组后台回归与 13 项任务回归通过，构建、17,719 条站内链接及公开页面泄漏检查通过。绑定与 DNS 生效前不将这些本地验证当作正式域名已经上线。
- Pages 使用 GitHub Actions；在仓库设置保存 Custom domain 后，DNSPod 配置四条 GitHub Pages apex A 记录及 `www` CNAME，既有 `api` 记录保持有效。待证书就绪开启 Enforce HTTPS，最终验证 apex、www、旧域名跳转与 API 连接。
