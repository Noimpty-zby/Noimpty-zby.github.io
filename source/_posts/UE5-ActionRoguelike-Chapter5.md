...
这一章最大的困难不是 C++，而是**蓝图里那一大坨连线**。第二节和第三节的控件图表节点密密麻麻，跟着老师一根根连出来，连完能跑，但完全不知道为什么。所以本篇专门花了大量篇幅拆解蓝图：每根线代表什么、每个节点从哪来、以及**下次怎么自己连出来**。
...
这一节课程里没有，是额外补的。看到"新建一个 ActorComponent"时，第一反应通常是：血量不就是个 `float` 吗，直接写在角色类里不行？
...
> **验证习惯**：日志里显示 `0.000000` 时，第一反应往往是"正好，clamp 好像生效了"。实际上那只是**碰巧打了 10 次 10 点伤害**。**编译通过 ≠ 正确，日志好看 ≠ 正确**——多打一发才是真正的验证。
...
这是本节最关键、也最容易迷惑的一段——**"Break Rogue Attribute Set"这个节点根本没人创建过，它哪来的？**
...
控件图表里节点密密麻麻，初看完全是懵的。关键是：**只有三个节点在执行链上**。
...
图里有两个都叫 `OnHealthChanged_事件` 的节点，长得完全不一样，很容易以为是画重复了：
