...
这一章最大的收获是意识到：**优化不是锦上添花，而是把之前偷的懒还回去。** 每一项优化都不是凭空冒出来的需求，而是某个早期决策留下的后果。
...
这一节课程里没有，是额外补的。第一次看到"第六章：优化"这个标题时，疑问通常是：**优化什么？性能吗？**
...
这正是优化最难被安排进日程的原因——它的产出不是一个可以演示的功能，而是一堆消失的问题。所以这一篇定了一条规矩：**每写一项优化，必须写清楚"不做会怎样"。** 写不出来，就说明还没真正理解为什么要做。
...
这在打**静止的大目标**（爆炸桶）时没问题。一旦目标会动、会躲、体积小，玩家就需要一个**开火前的预测反馈**：现在按下去，弹会落在哪。
...
**理由三：注释掉再取消注释是最糟的方案。** 需要看就取消注释，编译一次；看完再注释回去，再编译一次。UE 一次编译动辄几十秒，一天下来能浪费掉半小时。而且极容易忘记注释回去，把调试代码提交进 git。
...
> **命名已经连着三节出问题了**（第三节 `SelectedActor` / `BestActor` 混用，本节 `TranceEnd` 和 `FAdjustLocation`，第五节还会有 `DistanceTo` 和 `BoxExtend`）。所以加了一条流程：**每节提交前扫一遍新增的变量名**，问三个问题——拼写对吗？前缀符合 UE 约定吗？名字说的是它实际存的东西吗？
...
更可取的是**方案一**，因为它表达的是硬约束——"背后的东西不该能交互"是规则，不是偏好。权重只应该用来在**合法候选之间**做细分。两个方案也可以叠加使用。
...
一个值得固定下来的动作：**写完循环先扫一遍循环体，逐行问"这一行的结果每次迭代会变吗"。** 不变的一律往外提。
