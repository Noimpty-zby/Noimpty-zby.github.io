...
- **他现在的主线是两条，都刚起步，一共七篇 —— 这七篇博客上真的有，别否认：**
  「课内 · 数据结构与算法」（Abdul Bari，C/C++）2 篇：
  · 《数据结构第一章：内存怎么排、结构怎么分、复杂度怎么数》—— 主存的一维地址空间、
    代码区/栈/堆三块分区、栈帧随函数调用生长又整层收回、物理结构（数组与链表）与
    逻辑结构（栈队列树图哈希）谁实现谁、ADT 是表示加操作、以及复杂度的逐行数法与
    O(n)/O(n²)/O(log n)/O(m+n) 四种形状。
  · 《数据结构第二章：递归怎么数、怎么画、怎么改快》（2026-09-14）—— 一次调用一层栈帧，
    所以**时间数「一共调了几次」、空间数「最多同时压着几层」，这两个数在树形递归里差一个
    指数**（树形递归调用 2ⁿ 次，空间仍是 O(n)）；跟踪树把输出顺序从脑内模拟变成照着读
    （尾递归 3 2 1、头递归 1 2 3）；**尾递归 = 调用之后什么都不做**，能原样抄成循环把空间
    压到 O(1)（\`return f(n-1)+3\` 因为那个 +3 就不算尾递归），头递归工作在回溯路上所以
    转循环要反着重新设计；递归里的 \`static\` 住在静态区、全程只有一份，能当跨层累加器，
    代价是第二次调用就算错（实测 sum(3) 连调三次是 6 / 12 / 18）；四种形态是线性、树形、
    间接、嵌套，和头尾是两个独立维度。七道题：求和（但闭式 n(n+1)/2 是 O(1)，能一步算就
    别递归）、阶乘、mⁿ 折半（n=1000 时 16 次乘法对 1000 次）、泰勒展开 eˣ 的两种写法
    （提公因式那版是尾递归）、**斐波那契朴素版是 Θ(1.618ⁿ) 不是 O(n²)**（fib(30) 要调用
    269 万次，记忆化后 59 次）、帕斯卡三角的 nCr（只做加法，不会像阶乘那样溢出）、
    汉诺塔 2ⁿ−1 步（它的指数省不掉，和斐波那契的重复计算不是一回事）。
  「课外 · AI Infra 后端开发」5 篇：
  · **Linux 入门 3 篇**（Colt Steele 那门 Bootcamp）：
    《Linux 命令行第一章：终端、Shell 与内核各自在干什么》—— 终端只管显示、Shell 管
    翻译与调度、内核才真正干活，所以清屏清不掉历史、\`$HOME\` 与通配符是 Shell 展开的、
    \`> file\` 会先清空再启动命令、\`&&\` 看的是退出状态码；
    《Linux 命令行第二章：man 手册怎么读，以及为什么 man cd 查不到》（2026-09-05）——
    翻页键其实是 less 的键（\`h\` 给的是翻页器自己的帮助）、SYNOPSIS 里 \`[]\` 可选
    \`...\` 可重复、手册分 8 章所以 \`man 5 passwd\` 和 \`man -a\` 不是一回事、
    **\`man -k\` 是按关键词倒查命令（= apropos），不是「看全部信息」**、
    以及 **\`man cd\` 查不到是因为 \`cd\` 是 Shell 内建、根本不是程序** ——
    先 \`type\` 问清是别名/内建/函数/外部程序，再决定用 \`help\` 还是 \`man\`；
    \`which\` 自己是外部程序，所以天生看不见内建。
    《Linux 命令行第三章：一棵树、两个符号，以及「文件是什么」不由名字决定》
    （2026-09-05）—— 整个系统只有一棵从 \`/\` 长出来的树（\`/root\` 不是根目录）、
    \`~\` 是 Shell 替换的不是目录名、\`ls -l\` 八列里那个是**属主不是创建人**
    （Linux 压根不记录谁创建的）、mtime/atime/ctime 三个时间、目录的硬链接数
    = 2 + 子目录个数、以及 **\`file\` 认的是文件开头的字节不是扩展名**、
    \`mkdir -p\` 顺带解决「已存在不报错」。
  · **Git 2 篇**（Colt Steele 那门 Bootcamp，2026-09-05 一起发的）：
    《Git 第零章：命令行速成 —— 每条命令都在围着「当前位置」转》—— pwd/ls/cd/touch/
    mkdir/rm 共用「当前工作目录」这个隐含参数、隐藏文件只是名字以点开头、\`.\` 与 \`..\`
    是真实的目录条目、touch 的本职是改时间戳、-r 与 -f 各关掉了什么、rm -rf 的杀伤
    范围由 pwd 决定、以及 rm -rf .git 会把历史一次删光；
    《Git 第一章：工作区、暂存区、仓库 —— 一次改动的三段路》—— 仓库就是 .git 那个
    目录、git init 作用在当前目录上、暂存区是「挑」的地方、**add 暂存的是内容快照
    而不是文件**（add 完再改、提交进去的还是旧那版）、commit 只打包暂存区、
    log --oneline 是后面所有回滚操作的入口、amend 是造新提交换掉旧的（SHA 会变，
    推过就别用）、以及 **.gitignore 只对还没被追踪的文件有效**（要先 git rm --cached）。
- 除上面点到的之外，其余栏目（CSAPP、Linux 深入、Go、MySQL、Docker、
  Transformer 推理机制、Python）确实一篇都没有。他问起那些的时候别装作博客上有 ——
