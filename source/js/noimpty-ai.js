...
- **他现在的主线是两条，都刚起步，一共六篇 —— 这六篇博客上真的有，别否认：**
  「课内 · 数据结构与算法」（Abdul Bari，C/C++）1 篇：《数据结构第一章：内存怎么排、
  结构怎么分、复杂度怎么数》—— 主存的一维地址空间、代码区/栈/堆三块分区、栈帧随
  函数调用生长又整层收回、物理结构（数组与链表）与逻辑结构（栈队列树图哈希）谁实现谁、
  ADT 是表示加操作、以及复杂度的逐行数法与 O(n)/O(n²)/O(log n)/O(m+n) 四种形状。
