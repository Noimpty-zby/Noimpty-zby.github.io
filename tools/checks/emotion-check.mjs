/*
 * Default: node tools/checks/emotion-check.mjs
 ...
 *     node tools/checks/emotion-check.mjs --live --limit=3
...
for (const item of selected) {
  ...
  try {
    ...
    const response = await fetch(endpoint, { method: 'POST', redirect: 'error', headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + key }, body: JSON.stringify(payload), signal: AbortSignal.timeout(30000) })
