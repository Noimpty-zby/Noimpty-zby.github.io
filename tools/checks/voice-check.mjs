 ...
 * 或者怀疑服务那边变了，手动跑一次。
 *
 *   node tools/checks/voice-check.mjs                 默认 12 次，测失控率和补尾音
 *   node tools/checks/voice-check.mjs -n 20           改次数
 *   node tools/checks/voice-check.mjs --samples out/  额外落盘音频，自己听
 *
...
export const parseOptions = argv => {
  ...
  let rounds = 12, sampleDir = null
  const seen = new Set()
  for (let i = 0; i < argv.length; i++) {
    const flag = argv[i]
    if (!['-n', '--samples'].includes(flag) || seen.has(flag))
      throw new Error('不支持或重复的参数：' + flag + '；使用 --help 查看用法')
    seen.add(flag)
    const value = argv[++i]
    if (!value || value.startsWith('-')) throw new Error(flag + ' 缺少有效值')
    if (flag === '-n') {
      if (!/^[1-9][0-9]*$/.test(value) || Number(value) > 100)
        throw new Error('-n 必须是 1–100 的整数')
      rounds = Number(value)
    } else sampleDir = value
  }
  return { rounds, sampleDir, help: false }
}

export const resultsPass = (rounds, completed, durations, padded, ceiling) =>
  completed === rounds && durations.length >= rounds && padded === durations.length &&
  durations.every(value => Number.isFinite(value) && value > 0 && value <= ceiling)

const main = async () => {
  const { rounds, sampleDir, help } = parseOptions(process.argv.slice(2))
  if (help) {
    console.log('用法: npm run voicecheck -- [-n 1..100] [--samples 目录]\n默认执行付费语音检查；--help 不读取密钥、不请求服务。')
    return
  }
  ...
  for (const emotion of [false, true]) {
    ...
    let leaked = 0, retried = 0, padded = 0, completed = 0, notices = []
    ...
    for (let round = 0; round < rounds; round++) {
      ...
      if (played.length && raw.length) completed++
    ...
    const ok = resultsPass(rounds, completed, durations, padded, CEILING)
    if (!ok) failures++
    console.log(`【${label}】`)
