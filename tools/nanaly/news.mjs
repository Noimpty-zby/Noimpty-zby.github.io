...
const attachSources = (md, hits) => {
  ...
  for (const raw of lines) {
    ...
    // 她正文里若还是自己写了链接或网址，一律清掉，只留我拼的这一个
    const text = sanitizeMd(m[3])
      .replace(/\[([^\]\n]*)\]\([^)\s]*\)/g, '$1')
      // 连着包住网址的括号一起清 —— URL 不能吃掉右括号，否则会剩一个孤零零的「（」
      .replace(/[[［【(（]?\s*https?:\/\/[^\s)）\]］】]*\s*[)）\]］】]?/g, '')
      // 她写的「来源 / 原文 / 链接」标签，各种括号形态一并清掉，只留我拼的那个
...
/* 系统提示里只放「她是谁」和「主人是谁」—— 这两样对四个主题完全一样，
 * 所以这一整块（实测约 950 token）能被后三次调用的缓存命中。
 *
 * 以前这里第四行写的是「你在给主人整理一份「${t.title}」的简报」：主题名排在
...
\`- [编号] **一句话标题** —— 正文\`
例如：\`- [3] **某公司裁了整个引擎组** —— ……\`
**不要自己写网址，不要写「来源」两个字**，链接我这边会自动挂上去。
...
3. **我的看法** —— 一句你自己的判断，要说得具体，并且**明确标出这是你的看法**
   （用「我觉得」开头）。不许写「值得关注」「值得学习」这种谁都能说的话
...
${recent ? `
【你最近干过的事】（你自己做的，写「我的看法」时可以自然带上，比如你刚给某篇写过批注）
${recent}` : ''}
素材：
${listed}`
...
> ${sanitizeMd((intro || '我把这三天值得看的都捞过来了喵。').trim())}
>
