...
export const decide = ({ state, pending, env = process.env }) => {
  ...
  if (active.length < MAX_ACTIVE) {
    ...
    if (pool.length >= SHORTLIST_AT && rounds >= MIN_EXPLORE_ROUNDS) {
      ...
      return {
        kind: 'explore', held,
        why: held
          ? '这批刚比过，第一名已经排在最前面 —— 立项闸门没开，先接着扫方向等着'
  ...
  /* 7. 兜底 → 探索
   *
   * ⚠️ why 里**不要**把 held.why 再抄一遍。
   * 页面上这两句是**上下相邻**的两行：why 在上，闸门那一块紧跟着在下面，
   * 而闸门那块已经完整地写了「差哪一条 + 怎么解开」。抄进来的结果是
   * 同一句话连着出现两次，中间还夹着两个破折号，读起来像卡带。
   * why 只回答「所以这一轮去干嘛」，理由交给下面那块。 */
  return {
    kind: 'explore', held,
    why: held ? '立项闸门没开 —— 这一轮接着扫方向，不浪费' : ''
  }
}
