...
await test('current and historical images plus PDF scans share one two-image request budget', async () => {
  const current = { ...picture, id: 'current-picture-001' }
  ...
  assert.match(parts.filter(part => part.type === 'text').map(part => part.text).join('\n'), /扫描图未发送、未读取/)
