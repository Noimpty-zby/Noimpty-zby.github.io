 ...
 *
 * 为什么需要它：线上出过一篇标题叫
 *   《我在服务器角落看了三天提交记录，主人像只被 UE5 追着跑的仓鼠。日程更新从 8 月 13 日一路滚到 16 日，中间还》
...
check('★ 正常情况：标题和正文都拆得出来', () => {
  ...
  assert.ok(r.content.startsWith('我扫了一眼'))
})
...
check('★ 标题栏里塞了一整段正文 → 丢掉（这就是线上那次的形态）', () => {
  const long = '我在服务器角落看了三天提交记录，主人像只被 UE5 追着跑的仓鼠。日程更新从 8 月 13 日一路滚到 16 日，中间还改了三次时区'
  assert.equal(parseColumn(`标题：${long}\n===正文===\n${BODY}`), null)
})

check('★ 标题里有句号 → 丢掉（那是一句话，不是标题）', () => {
  assert.equal(parseColumn(`标题：主人这周很忙。我很闲。\n===正文===\n${BODY}`), null)
})
