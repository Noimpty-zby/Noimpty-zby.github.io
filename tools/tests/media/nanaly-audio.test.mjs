...
await test('zero streaming sizes are measured consistently with WAV normalization', async () => {
  const bytes = make({ data: Buffer.alloc(48000) })
  bytes.writeUInt32LE(0, 4); bytes.writeUInt32LE(0, 40)
  assert.equal(await measure(blob(bytes)), 1)
  assert.equal(await measure(await padWav(blob(bytes))), 1.35)
})
await test('oversized WAV input is rejected before allocating its array buffer', async () => {
  let reads = 0
  const huge = { size: 12 * 1024 * 1024 + 1, type: 'audio/wav', arrayBuffer() { reads++; throw Error('must not read') } }
  await assert.rejects(padWav(huge), /安全范围/)
  assert.equal(await measure(huge), null)
