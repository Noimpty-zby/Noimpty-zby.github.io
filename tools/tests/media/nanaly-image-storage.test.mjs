...
class Element {
  constructor(tag) { this.tagName = tag; this.className = ''; this.children = []; this.parentNode = null; this.attributes = new Map(); this.listeners = new Map(); this.hidden = false; this.isConnected = true }
...
const boot = (entries, saved = workspace(), { writable = true, encoded = 'data:image/jpeg;base64,AA==' } = {}) => {
  ...
  const database = { transaction() {
    const tx = { objectStore: () => ({
      ...
      put(item) { if (!writable) throw Error('storage full'); data.set(item.id, clone(item)); return request(item.id) }
    }) }
  ...
  const document = { createElement: tag => {
    ...
    if (tag === 'canvas') { node.getContext = () => ({ fillRect() {}, drawImage() {} }); node.toDataURL = () => encoded }
    return node
  } }
  class Image {
  ...
  vm.runInNewContext(source, { window, indexedDB, document, URL: { createObjectURL: () => 'blob:test', revokeObjectURL() {} }, Image, crypto, Date: FakeDate })
...
await check('malformed or oversized stored image data never reaches previews or model requests', async () => {
  for (const dataURL of ['https://outside.invalid/tracker.jpg', 'data:image/svg+xml;base64,AA==', 'data:image/jpeg;base64,' + 'A'.repeat(2600001)]) {
    const item = { ...image('image-corrupt-001'), dataURL }, h = boot([item]), mounted = h.mount()
    await mounted.ui.restore([ref(item)])
    assert.equal(mounted.ui.refs().length, 0)
    const node = new Element('div'); await h.api.decorate(node, [ref(item)])
    assert.equal(node.querySelectorAll('img').length, 0)
    await assert.rejects(h.api.imageContent('看看', [ref(item)]), /已不在本机/)
  }
})
await check('cyclic caller state cannot overflow attachment pruning', async () => {
  const item = image('image-cycle-001'), h = boot([item]), state = workspace([{ attachments: [ref(item)] }])
  state.self = state
  await h.api.prune(state)
  assert.equal(h.data.has(item.id), true)
})
await check('invalid file byte lengths fail validation before decoding an image', () => {
  const h = boot([])
  for (const size of [NaN, Infinity, -1, 0, 1.5, '42']) assert.throws(() => h.api.validateFile({ type: 'image/png', size }), /10 MB/)
})



await check('full temporary image storage rejects new uploads without evicting earlier unsaved images', async () => {
  const encoded = 'data:image/jpeg;base64,' + 'A'.repeat(2500000)
  const h = boot([], workspace(), { writable: false, encoded }), mounted = h.mount()
  const picker = mounted.panel.querySelectorAll('input').find(node => node.type === 'file')
  let first
  for (let i = 0; i < 12; i++) {
    mounted.ui.clear()
    picker.files = [{ name: 'screenshot.png', type: 'image/png', size: 100 }]
    picker.onchange(); await tick()
    if (i === 0) first = mounted.ui.refs()[0]
  }
  assert.ok(first)
  assert.ok(mounted.notices.some(message => /暂存已满/.test(message)))
  assert.equal(mounted.ui.refs().length, 0)
  const content = await h.api.imageContent('早先那张仍在', [first])
