...
await test('实机检查脚本的垫片仍能把真模块启动起来', async () => {
  // tools/checks/voice-check.mjs 是唯一能照出「服务那边变了」的检查，而它全靠那份垫片。
  // 模块哪天用上一个新的全局对象，垫片就再也启动不了 —— 偏偏那种时候最需要它。
  const { loadModules } = await import('../../checks/voice-check.mjs')
...
await test('invalid speech split sizes fail instead of hanging on empty or surrogate segments', () => {
  const h = boot()
  for (const limit of [0, -1, 1, 1.5, NaN, Infinity]) assert.throws(() => h.api.splitText('😀测试', limit), /分段长度/)
  assert.equal(h.api.splitText('😀测试', 2).join(''), '😀测试')
})
await test('oversized streaming audio is cancelled before playback or full-body buffering', async () => {
  let cancelled = 0, produced = 0
  const h = boot({ fetcher: async () => new Response(new ReadableStream({
    pull(controller) { produced++; controller.enqueue(new Uint8Array(1024 * 1024)) },
    cancel() { cancelled++ }
  }), { headers: { 'content-type': 'audio/wav' } }) })
  assert.equal(await h.controller.speak('太大的音频不加载。'), false)
  assert.equal(cancelled, 1)
  assert.ok(produced <= 14, 'stream is bounded while reading, not after consuming it all')
  assert.equal(h.audio.length, 0)
  assert.ok(h.notices.some(text => /音频过大/.test(text)))
})
await test('stopping while an audio stream is stalled cancels its reader and settles the request', async () => {
  let cancelled = 0
  const h = boot({ fetcher: async () => new Response(new ReadableStream({ cancel() { cancelled++ } }),
    { headers: { 'content-type': 'audio/wav' } }) })
  const reading = h.controller.speak('取消等待。'); await flush()
  h.controller.stop()
...
await test('a broken optional voice subscriber cannot strand playback or prevent cancellation', async () => {
  const h = boot()
  h.controller.subscribe(() => { throw Error('broken avatar') })
  const job = h.controller.speak('其他组件出错仍然可以停止。'); await flush()
  assert.equal(h.audio.length, 1)
  h.controller.stop()
  assert.equal(await job, false)
  assert.equal(h.urls.size, 0)
