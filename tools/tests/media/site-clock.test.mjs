...
const bootDom = (startMs = at(2026, 9, 20, 15, 42, 7).getTime(), { built = true } = {}) => {
  ...
  const doc = {
    ...
    querySelector: sel => atHome && sel.includes('site-info') ? hero : null,
    getElementById: id => atHome && hero.all().find(n => n.id === id) || null,
    addEventListener: (t, fn) => { (listeners[t] ||= []).push(fn) },
    removeEventListener: (t, fn) => { const a = listeners[t] || []; const i = a.indexOf(fn); if (i >= 0) a.splice(i, 1) }
  }
  const win = { addEventListener: (t, fn) => { (windowListeners[t] ||= new Set()).add(fn) }, removeEventListener: (t, fn) => windowListeners[t]?.delete(fn) }
  vm.runInNewContext(source, {
  ...
  return {
    hero, doc, finishHub, api: win.NOIMPTY_CLOCK, pending, listeners, windowListeners,
    leave: () => { atHome = false },
...
test('leaving the homepage through PJAX tears down the old clock immediately', () => {
  const d = bootDom()
  d.leave(); d.api.mount()
  assert.equal(d.pending.length, 0)
  assert.equal(d.listeners.visibilitychange.length, 0)
  assert.equal(d.windowListeners.pageshow.size, 0)
})
test('BFCache pageshow resumes the stopped clock exactly once', () => {
  const d = bootDom()
  d.windowListeners.pagehide.forEach(fn => fn())
  assert.equal(d.pending.length, 0)
  d.windowListeners.pageshow.forEach(fn => fn())
  d.windowListeners.pageshow.forEach(fn => fn())
  assert.equal(d.pending.length, 1)
  d.step(1); assert.equal(d.slot('ss').textContent, '08')
})
