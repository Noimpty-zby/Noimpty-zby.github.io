...
check('改文字与远端自动完成可以同时保留', () => {
  const base = { d: [T('a', '旧文字')] }
  const mine = { d: [T('a', '新文字')] }
  const theirs = { d: [T('a', '旧文字', { done: true, autoAt: '2026-09-25T01:00:00Z', autoWhy: '发了新文章' })] }
  const task = find(mergeDays(base, mine, theirs), 'd', 'a')
  assert.equal(task.text, '新文字')
  assert.equal(task.done, true)
  assert.equal(task.autoWhy, '发了新文章')
  assert.equal(task.autoAt, '2026-09-25T01:00:00Z')
})
check('本地勾选与远端改文字可以同时保留', () => {
  const base = { d: [T('a', '旧文字')] }
  const mine = { d: [T('a', '旧文字', { done: true })] }
  const theirs = { d: [T('a', '远端新文字')] }
  const task = find(mergeDays(base, mine, theirs), 'd', 'a')
  assert.equal(task.text, '远端新文字')
  assert.equal(task.done, true)
})
check('本地改文字不会撤销远端移动日期', () => {
  const base = { d: [T('a', '旧文字')] }
  const mine = { d: [T('a', '新文字')] }
  const theirs = { tomorrow: [T('a', '旧文字')] }
  const out = mergeDays(base, mine, theirs)
  assert.equal(out.d, undefined)
  assert.equal(find(out, 'tomorrow', 'a').text, '新文字')
})
check('本地更换判据不会混入远端旧判据的自动完成', () => {
  const base = { d: [T('a', '写笔记', { when: { type: 'section', match: 'Git' } })] }
  const mine = { d: [T('a', '写笔记', { when: { type: 'section', match: 'Go' } })] }
  const theirs = { d: [T('a', '写笔记', { when: { type: 'section', match: 'Git' }, done: true, autoAt: '2026-09-25', autoWhy: 'Git更新' })] }
  const task = find(mergeDays(base, mine, theirs), 'd', 'a')
  assert.equal(task.done, false)
  assert.equal(task.autoAt, undefined)
  assert.equal(task.when.match, 'Go')
})


check('本地只移日期时保留远端新判据并失效旧日期完成', () => {
  const original = T('a', '写笔记', { when: { type: 'section', match: 'Git' } })
  const remote = { ...original, when: { type: 'section', match: 'Go' }, done: true, autoAt: '2026-09-25', autoWhy: 'Go更新' }
  const task = find(mergeDays({ d: [original] }, { tomorrow: [original] }, { d: [remote] }), 'tomorrow', 'a')
  assert.equal(task.when.match, 'Go')
  assert.equal(task.done, false)
  assert.equal(task.autoAt, undefined)
  assert.equal(task.autoWhy, undefined)
})
check('任一端移动已自动完成任务都不能继承原日期证据', () => {
  const original = T('a', '写笔记', { done: true, autoAt: '2026-09-25', autoWhy: 'Git更新' })
  const base = { d: [original] }, moved = { tomorrow: [original] }
  for (const [mine, theirs] of [[moved, base], [base, moved]]) {
    const task = find(mergeDays(base, mine, theirs), 'tomorrow', 'a')
    assert.equal(task.done, false)
    assert.equal(task.autoAt, undefined)
    assert.equal(task.autoWhy, undefined)
  }
  assert.equal(original.done, true, '不得修改输入数据')
  assert.equal(original.autoAt, '2026-09-25')
})
check('移日保留纯手工完成和新日期重新产生的自动完成', () => {
  const original = T('a', '写笔记', { done: true })
  const task = find(mergeDays({ d: [original] }, { tomorrow: [original] }, { d: [original] }), 'tomorrow', 'a')
  assert.equal(task.done, true)
  const before = { ...original, autoAt: '2026-09-25', autoWhy: '旧记录' }
  const after = { ...original, autoAt: '2026-09-26', autoWhy: '新记录' }
  const refreshed = find(mergeDays({ d: [before] }, { d: [before] }, { tomorrow: [after] }), 'tomorrow', 'a')
  assert.equal(refreshed.done, true)
  assert.equal(refreshed.autoAt, '2026-09-26')
  assert.equal(refreshed.autoWhy, '新记录')
})
check('移日清除旧撤销标记但保留明确重新手工勾选', () => {
  const original = T('a', '写笔记', { autoAt: '2026-09-25', autoWhy: '旧记录' })
  for (const done of [false, true]) {
    const task = find(mergeDays({ d: [original] }, { tomorrow: [{ ...original, done }] }, { d: [original] }), 'tomorrow', 'a')
